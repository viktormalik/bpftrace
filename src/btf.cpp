...
{
  if (!has_data()) {
    return make_error<ast::ProbeArgParseError>(func, "BTF data not available");
  }
  ...
  if (!t || !btf_is_func_proto(t)) {
    return make_error<ast::ProbeArgParseError>(func, "not a function");
  }
  ...
  for (__u16 j = 0; j < vlen; j++, p++) {
    ...
    if (!str) {
      return make_error<ast::ProbeArgParseError>(func,
