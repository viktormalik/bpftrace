...
char ProbeArgParseError::ID;

void ProbeArgParseError::log(llvm::raw_ostream &OS) const
{
    ...
    const AttachPoint &ap)
{
  ...
  switch (probe_type) {
    ...
    case ProbeType::tracepoint: {
      ...
      if (!args)
        return make_error<ast::ProbeArgParseError>(ap.name(), "args not found");
      return args;
    }
    case ProbeType::uprobe: {
      ...
      if (dwarf) {
        ...
        if (args && args->fields.size() >= arch::Host::arguments().size()) {
          return make_error<ast::ProbeArgParseError>(
              ap.name(),
      ...
      return make_error<ast::ProbeArgParseError>(ap.name(),
